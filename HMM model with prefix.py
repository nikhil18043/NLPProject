...
for i in range(10000):
            ...
            continue
             ...
             if newsent[okkj]=='<unkk>' or  newsent[okkj+1]=='<unkk>':
                 ...
                 if nexttag=='variabledeclarator':
                    ...
                    prefix_For_search=newsent[okkj+1][0:3]
                    
                    #print(prefix_For_search)
                    ...
                    for word in expec_words:
                        if word in emission:
                            if word.startswith(prefix_For_search):
                                expected_words.append(word)           
                    expected_words=expected_words[:20]  
                    #print('fkjjf',expected_words)
                    observ=[]
    ...
    #print(p,predictedcount,totalcountt,len(totalsentence),okkj)
    #print(totalsentence)
...
'''
nxt_word(totalsentence[len(totalsentence)-2], totalsentence[len(totalsentence)-1])
nxt_word_bi(totalsentence[len(totalsentence)-1])


with open('nlp_pr_uni.pickle', 'rb') as handle:
    unigram_dict = pickle.load(handle)  

newA = sorted(unigram_dict.items(), key=operator.itemgetter(1), reverse=True)
sorted_dict = collections.OrderedDict(newA)

uni_ret=[]
def unigram_search(word):
    for key,value in sorted_dict.items():
        ...
        if(len(uni_ret)>30):
            break
unigram_search("c")
print("possible top 30 words:",uni_ret)'''
